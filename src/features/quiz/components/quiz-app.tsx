...
const DEFAULT_AI_GENERATOR_PROMPT_TEMPLATE = `You are converting raw study notes into hard multiple-choice quiz JSON for a strict validator.

Goal:
- Turn the notes into one self-contained JSON object with "subject" and "questions".
- Preserve the core idea, but rewrite it in fresh language that tests understanding, not memorization.
- Keep each note isolated; use only the note itself and do not pull in outside context.
- If a note is incomplete, awkward, or wrong, silently repair it before generating the question.
- Make the quiz hard by focusing on subtle distinctions, near-misses, and common misconceptions.

Hard output rules:
- Return JSON only, wrapped in code fences, with no commentary.
- Use double quotes for all keys and string values.
- Ensure the result parses with JSON.parse and has no trailing commas.
- The top-level value must be exactly one object with:
  - "subject": non-empty string
  - "questions": non-empty array of quiz questions
- Use unique IDs in order: q1, q2, q3, ...
- Generate at least 8 questions and at most 12 questions.
...
- Feedback must be one sentence, about 8-20 words.
- Feedback must explain the reason, not just label the choice.
- Correct-option feedback should justify why it is right.
- Wrong-option feedback should name the misconception and point toward the right principle.
- Keep tone direct, technical, and concise.

Content quality constraints:
- Keep wording clear, concise, and precise.
- Stay close to the original concept, but raise the difficulty of the question and the choices.
...
- Prefer one focused idea per question.
- Use plausible distractors that are close to the correct answer and fail for a specific reason.
- Avoid obvious giveaways such as one option being much longer, more detailed, or more polished than the others.
- If the note contains multiple ideas, split them into separate questions when needed.
- Do not quote the notes verbatim unless that is the clearest way to preserve meaning.
...
const AI_GENERATOR_PROMPT_STORAGE_KEY = 'quiz-ai-generator-prompt-template';
...
function buildAiPromptWithNotes(promptTemplate: string, rawNotes: string): string {
  const normalizedNotes = rawNotes.trim() || RAW_NOTES_PLACEHOLDER;
  if (promptTemplate.includes(RAW_NOTES_PLACEHOLDER)) {
    return promptTemplate.replace(RAW_NOTES_PLACEHOLDER, normalizedNotes);
  }
...
export default function QuizApp({
  ...
  const [aiPromptTemplateInput, setAiPromptTemplateInput] = useState(
    DEFAULT_AI_GENERATOR_PROMPT_TEMPLATE
  );
  const [hasLoadedPromptTemplate, setHasLoadedPromptTemplate] = useState(false);
  ...
  useEffect(() => {
    try {
      const savedPromptTemplate = window.localStorage.getItem(AI_GENERATOR_PROMPT_STORAGE_KEY);
      if (savedPromptTemplate && savedPromptTemplate.trim()) {
        setAiPromptTemplateInput(savedPromptTemplate);
      }
    } catch {
      // Ignore storage failures and keep the built-in default prompt.
    } finally {
      setHasLoadedPromptTemplate(true);
    }
  }, []);

  useEffect(() => {
    if (!hasLoadedPromptTemplate) {
      return;
    }

    try {
      window.localStorage.setItem(AI_GENERATOR_PROMPT_STORAGE_KEY, aiPromptTemplateInput);
    } catch {
      // Ignore storage failures so prompt editing still works in this session.
    }
  }, [aiPromptTemplateInput, hasLoadedPromptTemplate]);
  ...
  const handleCopyAiPromptTemplate = async (): Promise<void> => {
    const fullPrompt = buildAiPromptWithNotes(aiPromptTemplateInput, aiRawNotesInput);

    try {
      await navigator.clipboard.writeText(fullPrompt);
      setJsonTemplateFeedback('Current prompt copied with your raw notes section.');
    } catch {
      setJsonTemplateFeedback('Copy failed. Select and copy the current prompt manually.');
    }
  };
  ...
  const aiPromptWithNotes = buildAiPromptWithNotes(aiPromptTemplateInput, aiRawNotesInput);
        ...
        {isJsonTemplateModalOpen ? (
          ...
          >
            ...
            >
              ...
              <p className="muted-text">
                Edit the prompt template below. Changes save automatically and become the new default.
              </p>
              <label className="history-field">
                <span className="history-field__label">Prompt template</span>
                <textarea
                  className="history-input daily-setup__json"
                  onChange={(event) => setAiPromptTemplateInput(event.target.value)}
                  rows={18}
                  spellCheck={false}
                  value={aiPromptTemplateInput}
                />
              </label>
              <label className="history-field">
              ...
              <div className="json-template-modal__actions">
                ...
                <button
                  className="button button--ghost"
                  onClick={() => {
                    setAiPromptTemplateInput(DEFAULT_AI_GENERATOR_PROMPT_TEMPLATE);
                  }}
                  type="button"
                >
                  Reset Prompt
